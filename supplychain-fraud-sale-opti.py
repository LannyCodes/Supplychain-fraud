...
def calculate_all_features_iv(X, y):
    """
    计算所有特征的IV值并排序
    ...
    for feature in X.columns:
        try:
            ...
            if X[feature].dtype in ['int64', 'float64']:
                ...
                _, iv = calculate_woe_iv(pd.DataFrame({feature: X_binned, 'target': y_binary}), feature, 'target')
            else:
                # 对于分类特征，直接计算
...
# ========== 加权投票（使用调优后F1作为权重） ==========
# 权重: [0.14285714285714288, 0.7142857142857143, 0.14285714285714288]
# 最优阈值: 0.22, 最优Fβ(β=2): 0.9260
# 混淆矩阵：
# [[43925   190]
#  [   49   966]]
# 准确率 (Accuracy): 0.9947
# 精确率 (Precision): 0.8356
# 召回率 (Recall): 0.9517
# F1分数 (F1-Score): 0.8899

# ========== 双模型投票（LightGBM + XGBoost） ==========
# 权重: [0.8, 0.19999999999999996]
# 最优阈值: 0.20, 最优Fβ(β=2): 0.9228
# 混淆矩阵：
# [[43951   164]
#  [   59   956]]
# 准确率 (Accuracy): 0.9951
# 精确率 (Precision): 0.8536
# 召回率 (Recall): 0.9419
# F1分数 (F1-Score): 0.8956
